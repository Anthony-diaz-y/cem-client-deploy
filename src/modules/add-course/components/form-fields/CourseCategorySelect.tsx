...
interface CourseCategorySelectProps<T extends FieldValues = FieldValues> {
  ...
  loading?: boolean;
  domainName?: string; // Filter categories by domain name (compatibility)
  categoryType?: "career" | "sector"; // NEW: Filter by explicit type
  required?: boolean;
}
...
>({
  ...
  const [prevInitialData, setPrevInitialData] = useState<string>(
    initialData || "",
  );
  ...
  const filteredCategories = categories.filter((category) => {
    ...
    const matchesDomain = targetDomainName
      ? categoryDomainName.includes(targetDomainName) ||
        targetDomainName.includes(categoryDomainName)
      : true;

    // NEW: Match by explicit type (career or sector)
    const matchesType = categoryType ? category.type === categoryType : true;
            ...
            {!loading &&
              filteredCategories.map((category) => (
                <div
                  ...
                  className={`mx-2 px-6 py-3 rounded-xl cursor-pointer transition-all font-medium ${
                    (category.id || category._id) === selectedCategoryId
