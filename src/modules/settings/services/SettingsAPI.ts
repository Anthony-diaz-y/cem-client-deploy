...
import { setUser } from "../../auth/store/profileSlice"
...
import { ApiError, NavigateFunction } from "../../auth/types"
...
export function updateUserProfileImage(token: string, formData: FormData) {
  return async (dispatch: AppDispatch) => {
    ...
    try {
      ...
      toast.error(apiError.response?.data?.message || "Could Not Update Profile Picture")
    }
...
export function updateProfile(token: string, formData: ProfileFormData) {
  return async (dispatch: AppDispatch) => {
    ...
    try {
      ...
      toast.error(apiError.response?.data?.message || "Could Not Update Profile")
    }
    toast.dismiss(toastId)
  }
}
...
// ================ change Password  ================
export async function changePassword(token: string, formData: PasswordFormData) {
  const toastId = toast.loading("Loading...")
  try {
...
export function deleteProfile(token: string, navigate: NavigateFunction) {
  return async (dispatch: AppDispatch) => {
    ...
    try {
      ...
      toast.error(apiError.response?.data?.message || "Could Not Delete Profile")
    }
    toast.dismiss(toastId)
  }
}