/**
 * Hook personalizado para manejar la lógica del modal de eliminación de categorías
 */
...
export function useDeleteCategoryModal({
  ...
  /**
   * Carga los cursos asociados y otras categorías disponibles
   */
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (categoriesResult) {
        ...
        // Inicializar la categoría seleccionada para "mover todos"
        if (others.length > 0) {
          setSelectedCategoryForAll(others[0].id);

          // Inicializar el mapa de categorías para cada curso
  ...
  /**
   * Cambia la categoría de un curso individual
   */
  ...
  /**
   * Abre el modal de confirmación para eliminar un curso individual
   */
  ...
  /**
   * Elimina un curso individual después de confirmación
   */
  ...
  /**
   * Mueve todos los cursos a la categoría seleccionada usando endpoint masivo
   */
  ...
  /**
   * Elimina la categoría (solo si no tiene cursos)
   */
  ...
  /**
   * Maneja la eliminación de todos los cursos de la categoría
   */
