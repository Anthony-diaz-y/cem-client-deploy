...
interface DeleteAllCoursesModalProps {
  isOpen: boolean;
  onClose: () => void;
  categoryName: string;
  coursesCount: number;
...
/**
 * Modal de confirmación para eliminar todos los cursos de una categoría
 * Incluye advertencia clara y confirmación antes de proceder
 */
export default function DeleteAllCoursesModal({
  isOpen,
  onClose,
  categoryName,
  coursesCount,
  onConfirm,
}: DeleteAllCoursesModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Error al eliminar los cursos");
  ...
  return (
    <div className="fixed inset-0 z-[1001] !mt-0 grid place-items-center overflow-auto bg-white bg-opacity-10">
        ...
        <div className="space-y-4 mb-6">
          <div>
            <p className="text-richblack-300 mb-2">
              ¿Estás seguro de que deseas eliminar <strong className="text-richblack-5">todos los {coursesCount} curso(s)</strong> de la categoría:
            </p>
          ...
          <div className="bg-yellow-500/10 border-l-4 border-yellow-500/30 p-4 rounded">
            <p className="text-yellow-400 text-sm">
              ...
              Todos los cursos de esta categoría serán eliminados permanentemente.
            </p>
          </div>
        </div>
